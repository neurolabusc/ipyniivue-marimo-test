...
NAV_OUTER = (
    '<div id="marimo-top-nav" style="position:fixed;left:0;right:0;top:0;'
    'z-index:2147483646;background:linear-gradient(90deg,#0b1220,#07101a);'
    'color:#fff;padding:10px 12px;font-family:system-ui,Arial;display:flex;gap:12px;'
    'align-items:center;box-shadow:0 2px 8px rgba(0,0,0,0.25);">'
    '<div style="font-weight:700;">niivue examples</div>'
    '<div style="margin-left:12px;font-size:0.95rem;color:#ddd;">Demo: <strong>{title}</strong></div>'
    '<div style="margin-left:auto;display:flex;gap:8px;">'
    '<a id="marimo-nav-prev" href="{prev}" style="color:#111;background:#ffd86b;padding:8px 10px;border-radius:6px;text-decoration:none;font-weight:600;">← Previous</a>'
    '<a id="marimo-nav-chooser" href="../" style="color:#fff;background:#888;padding:8px 10px;border-radius:6px;text-decoration:none;font-weight:600;">Chooser</a>'
    '<a id="marimo-nav-next" href="{next}" style="color:#fff;background:#0b7;padding:8px 10px;border-radius:6px;text-decoration:none;font-weight:600;">Next →</a>'
    '</div></div>'
)
...
(function(){
  ...
  function ensureStyleVar() {
    ...
    if (!document.getElementById("marimo-nav-style")) {
      ...
      st.textContent = ':root{--marimo-top-offset:56px;}';
